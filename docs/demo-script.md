...
All three terminal screens also showed `✓ VERIFIED`, `3 VALID` evidence references, eight command facts, `✓ ABSENT`, `4 EXACT`, and `LOCKED · NOT REQUESTED`. The revised 249-word neural narration renders in 2:18.17, so the storyboard itself—not only the fast automated UI execution—fits the three-minute limit with more than 41 seconds of margin.

## Local review cut

Issue #17 renders concise narration and actual UI captures into an ignored local MP4. The current 2:18.17 review cut uses the warm, confident Andrew neural voice. Its nine narration clips map one-to-one to the nine slides, so each transition occurs only after the matching spoken beat. It is 1280×720 H.264 with one mono AAC narration track and is available through the [unlisted YouTube review link](https://youtu.be/9PyrTSgSAhU). See [submission video assets](../submission/README.md) to reproduce it. Public visibility still requires explicit approval.
