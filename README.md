...
> Milestone 3 in progress: exact-plan approval gates an isolated worktree, the approved json5 update is applied there, and one bounded compatibility repair now produces a syntax-checked three-file review diff.
...
Open `http://127.0.0.1:4173`, choose **Run deterministic scan**, **Investigate affectedness**, and **Review remediation plan**. Approve the exact plan, choose **Create isolated workspace**, **Apply approved dependency update**, then **Repair source compatibility**. The expected result is `GHSA-9c47-m6qq-7p4h` in direct dependency `json5@1.0.1`, evidence at `src/theme.js:1` and `src/theme.js:8–10`, a `likely affected` interpretation, and an isolated three-file diff. The manifest and lockfile move to `json5@1.0.2`; `parseUserTheme` copies only the supported `accent` and `density` fields; the bounded syntax probe passes; and the source checkout remains unchanged.
...
The proposal remains `awaiting_approval`. Approval and cancellation are recorded only in server memory for the current demo run. After approval, isolation records baseline, branch, worktree, and validated paths. The dependency step executes only the displayed npm command. The repair step can replace only the exact `parseUserTheme` function, runs `node --check src/theme.js` after each attempt, and stops after at most two failures while restoring the original source. Both steps preserve validated audit artifacts under ignored `runs/`. Targeted tests, the full suite/build, rescan, and report remain later checkpoints.
